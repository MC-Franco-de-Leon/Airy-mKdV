...
% writelatextable( [], M, '|c|c|' , [], [] );
   ...
   % m = [(10:-1:1)' rand([10,2])];
    ...
    h(2).text='$||\theta_{dt}-\theta_{dt/2}||_{l_2}$'; h(2).format='c|||'; 
    h(2).rows=[1];                h(2).cols=[2]; 
    % The last column has a subheading called '$NO_2$'
    h(3).text='$||\theta_{dt/2}-\theta_{dt/4}||_{l_2}$';           h(3).format='c|'; 
    h(3).rows=[1];                h(3).cols=[3];
    
    h(4).text='$||\theta_{dt/4}-\theta_{dt/8}||_{l_2}$';           h(4).format='c|'; 
    h(4).rows=[1];                h(4).cols=[4];
    
    h(5).text='$||\theta_{dt/8}-\theta_{dt/16}||_{l_2}$';           h(5).format='c|'; 
    h(5).rows=[1];                h(5).cols=[5];
    
    h(6).text='$||\theta_{dt/16}-\theta_{dt/32}||_{l_2}$';           h(6).format='c|||';
    h(6).rows=[1];                h(6).cols=[6];    
    
  
    % make a caption and label
    c = 'Time Convergence Error';
    l = 'table:atm_prof';
